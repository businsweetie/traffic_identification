...
Основано на файле `all_results.csv` и новом эксперименте с входами `CDF`, `PDF`, `Статистики`, `Гистограмма`.
...
```text
λ = 0,010; 0,100; 1,000; 2,000; 10,000
```
...
Главный результат нового эксперимента: после перехода к `log(λ)` и отказа от сырых моментов/интервалов качество ML- и NN-моделей существенно улучшилось. Особенно заметное улучшение наблюдается при `λ = 0,010`, где в предыдущем анализе ML и NN давали большие ошибки.
...
| 0,100  | 0,008                | 0,006     | 0,008     |
| 1,000  | 0,008                | 0,007     | 0,009     |
| 2,000  | 0,008                | 0,007     | 0,014     |
| 10,000 | 0,008                | 0,005     | 0,011     |

### Лучший метод для каждого λ и подхода

| λ      | Подход        | Лучший метод   | Вход        | RelErr   |
|:-------|:--------------|:---------------|:------------|:---------|
| 0,010  | ML            | RandomForest   | CDF         | 0,008    |
| 0,010  | NN            | LSTM           | Гистограмма | 0,011    |
| 0,010  | Аналитический | MM             | Интервалы   | 0,008    |
| 0,100  | ML            | LightGBM       | Статистики  | 0,006    |
| 0,100  | NN            | LSTM           | PDF         | 0,008    |
| 0,100  | Аналитический | L-момент       | Интервалы   | 0,008    |
| 1,000  | ML            | XGBoost        | Статистики  | 0,007    |
| 1,000  | NN            | LSTM           | Гистограмма | 0,009    |
| 1,000  | Аналитический | MLE            | Интервалы   | 0,008    |
| 2,000  | ML            | RandomForest   | Статистики  | 0,007    |
| 2,000  | NN            | LSTM           | Статистики  | 0,014    |
| 2,000  | Аналитический | MLE            | Интервалы   | 0,008    |
| 10,000 | ML            | XGBoost        | Статистики  | 0,005    |
| 10,000 | NN            | CNN1D          | Статистики  | 0,011    |
| 10,000 | Аналитический | MLE            | Интервалы   | 0,008    |
...
Метод `AD` после исправления также стал работать корректно. Раньше он давал огромные ошибки из-за некорректного диапазона оптимизации и особенностей минимизации. Сейчас его RelErr находится примерно на уровне `0,008–0,009`.
...
В предыдущем анализе ML и NN сильно ошибались при малых значениях `λ`, особенно при `λ = 0,010`.
...
|:-------|:-----------------|:----------------|:--------------------|:-----------------|:----------------|:--------------------|
| 0,010  | 2,002            | 0,008           | 256,233             | 4,441            | 0,011           | 390,871             |
| 0,100  | 0,035            | 0,006           | 5,855               | 0,075            | 0,008           | 9,099               |
| 1,000  | 0,007            | 0,007           | 1,090               | 0,014            | 0,009           | 1,632               |
| 2,000  | 0,008            | 0,007           | 1,198               | 0,010            | 0,014           | 0,752               |
| 10,000 | 0,009            | 0,005           | 1,674               | 0,009            | 0,011           | 0,800               |

### Что изменилось

Раньше:

- ML и NN резко ухудшались при `λ = 0,010`;
...
10. Если обучающий диапазон действительно равен `[0,005; 20,000]`, то значения `λ = 0,010` и `λ = 10,000` уже не являются экстраполяцией. График про экстраполяцию лучше переименовать в «Устойчивость моделей на широком диапазоне λ».

## 11. Что нужно улучшить дальше

1. Проверить, почему в `all_results.csv` нет `CatBoost`.
2. Оставить `Статистики` как основной вход для ML-моделей.
3. Оставить `LSTM` как основную NN-модель.
4. `DeepSets` оставить только в сравнительном анализе.
5. Для финальных таблиц использовать:
   - лучший RelErr;
   - средний RelErr;
   - минимальный RelErr;
   - максимальный RelErr.
6. Для текста диссертации писать, что ML приблизился к аналитическому ориентиру, а не превзошёл теоретическую границу.

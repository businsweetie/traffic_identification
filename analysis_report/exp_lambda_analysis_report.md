...
Генерация для обучения в диапазоне [0.100;10]
...
| λ | Статус |
|---:|---|
| 0.010 | ниже обучающего диапазона |
| 0.100 | внутри обучающего диапазона |
| 1.000 | внутри обучающего диапазона |
| 2.000 | внутри обучающего диапазона |
| 10.000 | внутри обучающего диапазона |
...
| Подход        |   λ=0.010 |    λ=0.100 |      λ=1 |      λ=2 |     λ=10 |
|:--------------|---------:|---------:|---------:|---------:|---------:|
...
- при `λ=0.100` ML и NN уже заметно хуже аналитики;
- при `λ=0.010` наблюдается сильное ухудшение, особенно у NN.
...
![RelErr оценки λ в зависимости от истинного значения λ](graphs/fig_relerr_vs_lambda.png)
...
![Аналитические методы: RelErr vs λ](graphs/fig_analytical_relerr_vs_lambda.png)
...
Для `λ=0.010`, `λ=0.100`, `λ=2`, `λ=10` лучший результат дают аналитические методы.
...
4. Среди ML:
   - `SVR` лучше всего работает на `λ=0.010`, `λ=2`, `λ=10`;
   - `RandomForest` лучший при `λ=0.100`;
...
![RelErr: метод × λ, лучший вход](graphs/fig_heatmap_method_lambda.png)

### График 4. Столбчатое сравнение всех методов по λ

![Сравнение всех методов по λ](graphs/fig_bar_all_methods_by_lambda.png)
...
| Вход                |    λ=0.010 |     λ=0.100 |      λ=1 |      λ=2 |     λ=10 |
|:--------------------|----------:|----------:|---------:|---------:|---------:|
...
![RelErr: вход × λ, лучшая ML/NN-модель](graphs/fig_heatmap_input_lambda.png)
...
|------:|:---------|:-------------|:-----------|---------:|
|  0.010 | ML       | SVR          | PDF (KDE)  | 2.002  |
|  0.100  | ML       | RandomForest | Статистики | 0.035 |
...
|:--------------------------------------------|:--------------|-------------------------------:|----------------------------:|-----------------:|
| как в CSV: λ=10 вне диапазона               | ML            |                       0.017 |                    1.005  |         59.623  |
| как в CSV: λ=10 вне диапазона               | NN            |                       0.033 |                    2.225  |         66.998   |
| как в CSV: λ=10 вне диапазона               | Аналитический |                       0.008 |                    0.008 |          1.017 |
| если считать λ=10 границей внутри диапазона | ML            |                       0.015 |                    2.002  |        135.467   |
| если считать λ=10 границей внутри диапазона | NN            |                       0.027 |                    4.441  |        163.659   |
| если считать λ=10 границей внутри диапазона | Аналитический |                       0.008 |                    0.008 |          1.046 |


### Интерпретация

Если считать по CSV, где `λ=10` отмечено как вне диапазона, то вне диапазона оказываются `λ=0.010` и `λ=10`.

Но фактически основная проблема связана именно с `λ=0.010`.
...
При `λ=0.010` качество резко падает:

- лучший ML: `RelErr=2.002`;
- лучший NN: `RelErr=4.441`;
- аналитика: `RelErr=0.008`.
...
При `λ=0.010` интервалы становятся в 10 раз больше, чем при нижней границе обучения `λ=0.100`. Если модель не видела таких масштабов, она плохо экстраполирует.

---


### График 6. Экстраполяция: работа моделей вне обучающего диапазона

![Экстраполяция: как модели работают вне обучающего диапазона](graphs/fig_extrapolation_analysis.png)
...
![Истинное vs предсказанное λ](graphs/fig_scatter_true_vs_pred.png)
...
Если `10.000` входит в диапазон:

```python
in_train_range = (lambda_true >= 0.1) & (lambda_true <= 10.0)
```

Если `10.000` не входит:
...
Методы машинного обучения способны приближаться к аналитическому уровню качества внутри обучающего диапазона и на верхней границе диапазона. Наилучшие результаты среди ML-моделей показали SVR, LightGBM и RandomForest. Однако при `λ=0.010` наблюдается резкое ухудшение качества, что связано с выходом за нижнюю границу обучающего диапазона и изменением масштаба межсобытийных интервалов.
...
4. NN менее устойчивы, особенно при `λ=0.010`.
